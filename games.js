...
window.setDiceBet = function (amount) {
    ...
    document.querySelectorAll('#modal-dice .casino-chip').forEach(chip => {
        if (Number(chip.dataset.amount) === Number(amount)) chip.classList.add('active');
...
async function rollDice() {
    ...
    if (die1) {
        die1.style.transition = 'transform 1.2s cubic-bezier(0.15, 0.85, 0.35, 1.2)';
    ...
    if (die2 && diceMode === 'double') {
        die2.style.transition = 'transform 1.2s cubic-bezier(0.15, 0.85, 0.35, 1.2)';
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            if (resultEl) {
                resultEl.textContent = res.outcome.label || `ROLLED ${val1}${diceMode === 'double' ? ' + ' + val2 : ''}`;
            }
    ...
    setTimeout(() => {
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = '<span class="btn-dice-icon">🎲</span> ROLL 3D CASINO DICE';
        }
    }, 1600);
}
}
...
async function pickCard(cardIndex) {
    if (typeof APP_STATE !== 'undefined' && !APP_STATE.isAuthenticated) {
        showToast('Please Register or Log In first to play Pick a Card!', 'warning');
        if (window.openAuthModal) window.openAuthModal('register');
        return;
    }
    ...
    const chosenCardEl = document.querySelector(`.play-card[data-index="${cardIndex}"]`);
    if (chosenCardEl) {
    ...
    try {
            ...
            res.cards.forEach((card, i) => {
                ...
                setTimeout(() => {
                    ...
                    if (cardEl) {
                        ...
                        renderCasinoCardFace(cardEl, card, card.winAmount > 0 || card.multiplier > 0, false);
                        playCardFlickSound();
                    }
                }, 180 * (i + 1));
            });
        ...
        showToast(err.message, 'error');
        cardsActive = true;
        if (hintEl) hintEl.textContent = '✨ Choose a card from the dealer\'s table to reveal your multiplier!';
    }
}
...
function resetCards() {
    ...
    if (hintEl) {
        ...
        hintEl.textContent = '✨ Choose a card from the dealer\'s table to reveal your multiplier!';
    }
...
window.playLucky7Slot = async function () {
    ...
    try {
        ...
        if (res.winAmount >= lucky7Bet * 25 || (res.chosen && res.chosen.type === 'jackpot')) {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                if (res.winAmount > 0) {
                    const isJackpot = targetSymbols[0] === '7️⃣' && targetSymbols[1] === '7️⃣' && targetSymbols[2] === '7️⃣';
                    if (resultEl) {
                    ...
                    playWinCoinSound();
                    triggerConfetti();
                    showWinModal(`KSh ${res.winAmount.toLocaleString()}`, `${targetSymbols.join(' ')} Slot Win`, res.xpGained);
                } else if (res.chosen && res.chosen.type === 'free_spin') {
                    if (resultEl) {
